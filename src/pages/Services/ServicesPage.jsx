...
import { MobileHeader, MobileSearch, MobileCategoryRail, MobileServiceCard, MobileBottomNav } from '../../components/mobile/MobileLayoutComponents';
...
const ServicesPage = () => {
    ...
    return (
        <>
            ...
            {/* Mobile View */}
            <div className="md:hidden fixed inset-0 z-40 bg-slate-50 overflow-y-auto safe-area-pb pb-24">
                <MobileHeader />
                <MobileSearch value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                <MobileCategoryRail categories={categories} selectedApi={selectedCategory} onSelect={setSelectedCategory} />

                <div className="px-4 py-4 grid gap-5">
                    <div className="flex items-center justify-between">
                        <h2 className="font-bold text-slate-900 text-lg">Recommended</h2>
                    ...
                    {filteredServices.length > 0 ? (
                        filteredServices.map(service => (
                            <MobileServiceCard key={service.id} service={service} onBook={handleBookClick} />
                        ))
                    ) : (
